...
export default function Modes() {
 ...
 const [buttonstate, changeBtnState] = useState("Light Mode");
  ...
  const modeFunc = () => {
    ...
    if (myStyles.color == "white") {
      ...
      changeBtnState("enable Dark Mode")
    }else{
      setMyStyles({
        color : "white",
        backgroundColor : "black"
      })
      changeBtnState("enable Light Mode")
    }
  }
  ...
  return (
    <div className="container my-5" style={myStyles}>
      ...
      <button className='btn btn-primary mt-5' onClick={modeFunc}>{buttonstate}</button>
    </div>
  );
}
